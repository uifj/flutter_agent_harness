...
| 路径 | 文件 | 行 | 职责 |
| --- | --- | --- | --- |
| `lib/main.dart` | 1 | ~250 | 纯装配：开 store → 恢复工作区授权 → 挂 scope。**不做决策** |
...
| `lib/state/` | 13 | 2.2k | 控制器与持久化：`app_scope`（运行时热替换）、`conversation_controller`、`workbench_controller` / `split_node` 布局、`layout_controller`、`settings_store` / `prefs_store`（原子写）、`streaming_tail` |
