...
适用场景：任何构建 / 打包 / 编译失败排查 / 发布相关任务。规则在 ponytail 与 workflow.md 之上，落地本仓库（`agent_harness`，macOS 桌面端）的真实构建事实；违反版本基线、擅动 entitlements、或在未真机验证前宣称出包成功，视为错误。

## 0. 版本基线与平台范围

- **Flutter 3.41.9 stable / Dart 3.11.5**（`pubspec.yaml` 要求 `sdk: ^3.11.5`）。不要为了"绕开一个报错"去换 SDK 版本；要升 SDK 先立 ADR。
- 只有 **`macos/`** 平台脚手架：仓库里没有 `android/`、`ios/`、`windows/`、`linux/`、`web/`。`flutter run -d windows` 必然失败（缺目录），别去补平台目录来"顺手支持跨平台"——那是 ARC 级决策。
- 源项目在 Mac 上用 fvm（`spike/*.dart` 的运行注释写着 `fvm dart run`）。本仓库**不含** fvm 配置文件：机器上有 fvm 就照注释用 fvm，没有就直接 `dart` / `flutter`，不要新引入 fvm 配置。
- 本机是 Windows：**禁止宣称 macOS 构建通过**。凡是没在 macOS 上真跑过的产物/步骤，交付说明里一律标"待 macOS 验证"。
- 依赖状态：仓库里 `pubspec.lock` 不入库（库型包约定，见 `.gitignore`），当前这台 Windows 机器上没有 `.dart_tool` 也没有 pub cache。所以**任何校验命令的第一步是 `flutter pub get`**；在拿到锁文件前，不要声称某个包"当前解析到的版本"是多少——用 `flutter pub deps` 现场核对。
...
- `flutter test` 报找不到包 / 缺 `.dart_tool` / pub cache 不存在：同样先 `flutter pub get`；这台 Windows 机从未解析过依赖。
...
- `xterm`、`flutter_pty`、`flutter_inappwebview`、`pasteboard` 都只有 macOS 实现路径；给它们加其他平台分支前，先回到 §0 的平台范围。
