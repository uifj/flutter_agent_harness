...
import 'package:board_panel/src/widgets/table_calendar/format_button.dart';
...
void main() {
  group('onTap callback tests:', () {
    testWidgets('Initial format month returns twoWeeks when tapped', (
      tester,
    ) async {
    ...
    testWidgets('Initial format twoWeeks returns week when tapped', (
      tester,
    ) async {
      const headerStyle = HeaderStyle();
      CalendarFormat? calendarFormat;
    ...
    testWidgets('Initial format week return month when tapped', (tester) async {
      const headerStyle = HeaderStyle();
      CalendarFormat? calendarFormat;
  ...
  group('showsNextFormat tests:', () {
    testWidgets('true - display next calendar format', (tester) async {
      const headerStyle = HeaderStyle();

      const currentFormatIndex = 0;
      final currentFormat = calendarFormatMap.keys.elementAt(
        currentFormatIndex,
      );
      final currentFormatText = calendarFormatMap.values.elementAt(
        currentFormatIndex,
      );

      const nextFormatIndex = 1;
      final nextFormatText = calendarFormatMap.values.elementAt(
        nextFormatIndex,
      );
    ...
    testWidgets('false - display current calendar format', (tester) async {
