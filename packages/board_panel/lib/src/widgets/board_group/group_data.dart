...
abstract class BoardPanelGroupItem extends ReoderFlexItem {
...
/// [BoardPanelGroupController] is used to handle the [BoardPanelGroupData].
///
...
class BoardPanelGroupController extends ChangeNotifier with EquatableMixin {
  BoardPanelGroupController({required this.groupData});

  final BoardPanelGroupData<dynamic> groupData;

  @override
  List<Object?> get props => groupData.props;

  /// Returns the readonly [UnmodifiableListView] of [BoardPanelGroupItem]
  UnmodifiableListView<BoardPanelGroupItem> get items =>
  ...
  ///
  BoardPanelGroupItem? removeAt(int index, {bool notify = true}) {
    ...
    Log.debug('[$BoardPanelGroupController] $groupData remove item at $index');
  ...
  void removeWhere(bool Function(BoardPanelGroupItem) condition) {
  ...
  bool move(int fromIndex, int toIndex) {
    ...
    Log.debug(
      '[$BoardPanelGroupController] $groupData move item from $fromIndex to $toIndex',
    );
  ...
  bool insert(int index, BoardPanelGroupItem item, {bool notify = true}) {
    assert(index >= 0);
    Log.debug('[$BoardPanelGroupController] $groupData insert $item at $index');
  ...
  bool add(BoardPanelGroupItem item, {bool notify = true}) {
    if (_containsItem(item)) {
      return false;
    } else {
      groupData._items.add(item);
  ...
  void replace(int index, BoardPanelGroupItem newItem) {
    if (groupData._items.isEmpty) {
      groupData._items.add(newItem);
      Log.debug('[$BoardPanelGroupController] $groupData add $newItem');
    } else {
      if (index >= groupData._items.length) {
        Log.error(
          '[$BoardPanelGroupController] unexpected items length, index should less than the count of the items. Index: $index, items count: ${items.length}',
        );
        return;
      }
      ...
      Log.debug(
        '[$BoardPanelGroupController] $groupData replace $removedItem with $newItem at $index',
      );
    }
  ...
  void replaceOrInsertItem(BoardPanelGroupItem newItem) {
  ...
  bool _containsItem(BoardPanelGroupItem item) {
...
/// [BoardPanelGroupData] represents the data of each group of the Board.
// ignore: must_be_immutable
class BoardPanelGroupData<CustomData> extends ReoderFlexItem
    with EquatableMixin {
  BoardPanelGroupData({
    required this.id,
    required String name,
    this.customData,
    List<BoardPanelGroupItem> items = const [],
  }) : _items = items,
       headerData = BoardPanelGroupHeaderData(groupId: id, groupName: name);

  @override
  final String id;
  BoardPanelGroupHeaderData headerData;
  final CustomData? customData;

  final List<BoardPanelGroupItem> _items;
...
class BoardPanelGroupHeaderData {
  BoardPanelGroupHeaderData({required this.groupId, required this.groupName});
