...
class WeekViewState<T extends Object?> extends State<WeekView<T>> {
                ...
                Expanded(
                  child: DecoratedBox(
                    decoration: BoxDecoration(
                      // Themed background only when the default is in use.
                      ...
                      child: PageView.builder(
                        ...
                        itemBuilder: (_, index) {
                          ...
                          return ValueListenableBuilder(
                            ...
                            builder: (_, _, _) => InternalWeekViewPage<T>(
                              key: ValueKey(
  ...
  void _updateViewDimensions() {
    ...
    _liveTimeIndicatorSettings =
        ...
        HourIndicatorSettings(
          color:
              ...
              Constants.defaultLiveTimeIndicatorColor,
    ...
    _hourIndicatorSettings =
        ...
        HourIndicatorSettings(
          height: widget.heightPerMinute,
          color: theme?.colorScheme.border ?? Constants.defaultBorderColor,
          offset: 5,
        );
  ...
  Widget _defaultWeekNumberBuilder(DateTime date) {
    ...
    return Center(child: Text('$weekNumber'));
  }
  ...
  Widget _defaultTimeLineBuilder(DateTime date) {
    final timeLineString =
        widget.timeLineStringBuilder?.call(date) ??
  ...
  Widget _defaultEventTileBuilder(
    ...
    if (events.isNotEmpty) {
      return RoundedEventTile(
        ...
        titleStyle:
            events[0].titleStyle ??
            TextStyle(fontSize: 12, color: events[0].color.accent),
    ...
    return WeekPageHeader(
      ...
      onTitleTapped: () async {
        if (widget.onHeaderTitleTap != null) {
          // Fire-and-forget by design: the header must not block on the
          // consumer's callback.
          unawaited(widget.onHeaderTitleTap!(startDate));
        } else {
