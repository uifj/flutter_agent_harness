...
class MergeEventArranger<T extends Object?> extends EventArranger<T> {
    ...
    for (final event in events) {
        ...
        if (!(event.endTime!.getTotalMinutes == 0 &&
          ...
          assert(() {
            try {
              ...
              // Suppress exceptions: debugPrint may throw in release-stripped
              // asserts; the surrounding assert only cares about returning
              // true so the event is skipped gracefully.
            }
      ...
      if (eventIndex == -1) {
        ...
        final newEvent = OrganizedCalendarEventData<T>(
          ...
          left: 0,
          right: 0,
          startDuration: arrangedEventData.startDuration.copyFromMinutes(
            startDuration,
          ),
          endDuration: arrangedEventData.endDuration.copyFromMinutes(
            endDuration,
          ),
  ...
  bool _checkIsOverlapping(
    ...
    var result =
        (arrangedEventStart >= eventStart && arrangedEventStart <= eventEnd) ||
