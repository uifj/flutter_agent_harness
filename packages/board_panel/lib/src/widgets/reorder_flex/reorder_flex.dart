...
abstract class ReorderDragTargetKeys {
  ...
  GlobalObjectKey? getDragTarget(String reorderFlexId, String key);
...
class ReorderFlexState extends State<ReorderFlex>
  ...
  void initState() {
    ...
    draggingState =
        widget.dragStateStorage?.readState(flexId) ??
    ...
    widget.reorderFlexAction?._scrollToBottom = scrollToBottom;

    widget.reorderFlexAction?._resetDragTargetIndex = resetDragTargetIndex;
    ...
    if (widget.autoScroll && _autoScroller == null) {
      _autoScroller = BoardDragAutoScroller(
        ...
        onScrollViewScrolled: () {
          final renderBox =
              draggingState.draggingKey?.currentContext?.findRenderObject()
                  as RenderBox?;
          if (renderBox != null) {
    ...
    return Builder(
      builder: (context) {
        ...
        if (shiftedIndex == currentIndex || childIndex == dragPhantomIndex) {
          ...
          final Widget disappearSpace = _makeDisappearSpace(
            dragSpace,
            feedbackSize,
          );
          ...
          if (draggingState.isPhantomAboveDragTarget()) {
            ...
            if (shiftedIndex == currentIndex &&
              ...
              return _buildDraggingContainer(
                children: [disappearSpace, dragTarget, appearSpace],
              );
            } else if (shiftedIndex == currentIndex) {
              return _buildDraggingContainer(
                children: [dragTarget, appearSpace],
              );
          ...
          if (draggingState.isPhantomBelowDragTarget()) {
            ...
            if (shiftedIndex == currentIndex &&
                childIndex == dragPhantomIndex) {
              return _buildDraggingContainer(
                children: [appearSpace, dragTarget, disappearSpace],
              );
            } else if (shiftedIndex == currentIndex) {
              return _buildDraggingContainer(
                children: [appearSpace, dragTarget],
              );
            } else if (childIndex == dragPhantomIndex) {
              return _buildDraggingContainer(
                children: shiftedIndex >= childIndex
  ...
  static ReorderFlexState of(BuildContext context) {
    ...
    final ReorderFlexState? result = context
        .findAncestorStateOfType<ReorderFlexState>();
    return result!;
  }
  ...
  void resetDragTargetIndex(int dragTargetIndex) {
    ...
    widget.dragStateStorage?.insertState(widget.reorderFlexId, draggingState);
  }
  ...
  bool handleOnWillAccept(BuildContext context, int dragTargetIndex) {
    ...
    final bool willAccept =
        draggingState.dragStartIndex == dragIndex &&
        dragIndex != dragTargetIndex;
    setState(() {
      if (willAccept) {
        final int shiftedIndex = draggingState.calculateShiftedIndex(
          dragTargetIndex,
        );
  ...
  void _scrollTo(BuildContext context) {
    ...
    // `offset` / `position` assert that exactly one scroll view is attached,
    // so `hasClients` alone is not a sufficient guard: a controller shared by
    // two live scroll views passes it and then trips the assertion.
