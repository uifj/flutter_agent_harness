...
typedef DragTargetWillAccepted<T extends DragTargetData> =
    bool Function(T dragTargetData);

typedef DragTargetOnStarted = void Function(Widget, int, Size?);

typedef DragTargetOnMove<T extends DragTargetData> =
    void Function(T dragTargetData, Offset offset);

typedef DragTargetOnEnded<T extends DragTargetData> =
    void Function(T dragTargetData);
  ...
  Widget _buildDraggableWidget(
    ...
    final feedbackBuilder = Builder(
      builder: (BuildContext context) {
        final BoxConstraints contentSizeConstraints = BoxConstraints.loose(
...
class _FakeDragTargetState<T extends DragTargetData>
  ...
  @override
  void dispose() {
    widget.insertAnimationController.removeStatusListener(
      _onInsertedAnimationStatusChanged,
    );
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    if (simulateDragging) {
      return SizeTransitionWithIntrinsicSize(
        ...
        child: AbsorbPointerWidget(opacity: 0.3, child: widget.child),
      );
    } else {
      return SizeTransitionWithIntrinsicSize(
