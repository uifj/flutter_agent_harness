...
class _ScrollShadowState extends State<ScrollShadow> {
  ...
  Widget build(final BuildContext context) {
    // Resolve the themed shadow color once per build; fall back to the
    // original hardcoded default when no ShadTheme ancestor exists or the
    // caller provided an explicit color.
    final effectiveColor = widget.color == Colors.black38
        ? (ShadTheme.maybeOf(
                context,
              )?.colorScheme.foreground.withValues(alpha: 0.38) ??
              widget.color)
        : widget.color;
    ...
    switch (_axis) {
      ...
      case Axis.horizontal:
        startGradient = LinearGradient(
          ...
          colors: [effectiveColor.withAlpha(0), effectiveColor],
        );
        endGradient = LinearGradient(
          begin: Alignment.centerRight,
          end: Alignment.centerLeft,
          colors: [effectiveColor, effectiveColor.withAlpha(0)],
        );
      case Axis.vertical:
        startGradient = LinearGradient(
